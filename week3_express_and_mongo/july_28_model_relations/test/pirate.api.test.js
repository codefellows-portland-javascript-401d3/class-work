...
// start the db connection, store so we 
// can close when done
// need mongoose to clean up schemas and models
...
describe( 'pirate', () => {
		...
		connection.on( 'open', () => {
			connection.db
				...
				.next( (err, collinfo) => {
					...
					connection.db.dropCollection( name, err => done() );
				});
		});
	});
	...
	// other options is to clean up after each test:
